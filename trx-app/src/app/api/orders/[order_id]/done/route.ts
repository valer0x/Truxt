...
export async function POST(
  ...
  try {
    ...
    if (verification.state !== "BOOKED") {
      return NextResponse.json(
        { error: `Order is ${verification.state} on network, expected BOOKED` },
        { status: 409 }
      );
    }

    // Verify carrier_did matches on network too
    if (verification.carrier_did !== actorDid) {
      return NextResponse.json(
        { error: "Network verification: you are not the carrier who booked this load" },
        { status: 403 }
      );
    }
